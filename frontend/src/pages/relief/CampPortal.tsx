...
import { campsApi, needsApi, pledgesApi } from '../../api/endpoints';
import type { Camp, ReliefNeed } from '../../types/relief';

// Ported from Disaster-Management-master CampPortal.tsx
// API: api.camps.getAll() → campsApi.getById() | api.needs → needsApi.getByCamp()
...
const CampPortal: React.FC = () => {
    ...
    const fetchCampDetails = async () => {
        try {
            const data = await campsApi.getById(campId!);
            setCamp(data);
        } catch (error) {
    ...
    const fetchCampNeeds = async () => {
        try {
            const data = await needsApi.getByCamp(campId!);
            setNeeds(data.filter((n: ReliefNeed) => n.isActive));
        } catch (error) {
    ...
    if (!camp) return (
        <div className="p-8 text-center bg-red-900/30 text-red-400 rounded-xl border border-red-500/40">
            ...
            <Link to="/relief" className="text-blue-400 underline">Return to Global Needs Registry</Link>
        </div>
    );
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {needs.length === 0 ? (
                    <div className="col-span-full p-8 text-center bg-emerald-900/30 text-emerald-400 rounded-xl border border-emerald-500/30">
                        ...
                        <Link to="/relief" className="inline-block mt-4 text-blue-400 hover:underline">View other camps in need</Link>
                    </div>
                ) : null}
                ...
                {needs.map((need, idx) => (
                    <div key={idx} className="border border-slate-700 rounded-xl shadow-lg bg-slate-800 overflow-hidden hover:shadow-xl transition flex flex-col">
                        {need.imageUrl && (
                            <img src={need.imageUrl} alt={need.itemName} className="w-full h-48 object-cover border-b border-slate-700" />
                        )}
                        {!need.imageUrl && (
                            ...
                            <div className="mb-4">
                                ...
                                <div className="w-full bg-slate-700 rounded-full h-2.5">
                                    <div className="bg-blue-500 h-2.5 rounded-full"
                                        style={{ width: `${Math.min((need.quantityPledged / need.quantityRequired) * 100, 100)}%` }}></div>
                                </div>
                            </div>
                            <button onClick={() => setPledgingNeed(need)} disabled={need.quantityPledged >= need.quantityRequired}
