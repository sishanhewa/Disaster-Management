...
const NeedsRegistry: React.FC = () => {
    ...
    const fetchNeeds = async () => {
        try {
            ...
            // Only show active needs, sorted by creation date (newest first)
            setNeeds(data.filter(n => n.active).sort((a,b) => new Date(b.createdAt || 0).getTime() - new Date(a.createdAt || 0).getTime()));
        } catch (error) {
    ...
    const filteredNeeds = needs.filter(need => {
        ...
        const needCategory = need.category || '';
        const matchesCategory = categoryFilter === 'All' || needCategory.toLowerCase() === categoryFilter.toLowerCase();
        
        const needUrgency = need.urgency || '';
        const matchesUrgency = urgencyFilter === 'All' || needUrgency.toLowerCase() === urgencyFilter.toLowerCase();
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
                        ...
                        return (
                            <div key={need.id} className="group flex flex-col bg-slate-800 rounded-2xl border border-slate-700 shadow-xl overflow-hidden hover:shadow-emerald-900/20 hover:border-slate-600 transition-all duration-300 transform hover:-translate-y-1">
                                <div className="relative h-48 overflow-hidden bg-slate-900">
                                    {need.imageBase64 ? (
                                        <img src={need.imageBase64} alt={need.itemName} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110 opacity-90 group-hover:opacity-100" />
                                    ) : (
                                    ...
                                    <div className="absolute top-3 right-3">
                                        <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider backdrop-blur-md border shadow-lg ${urgencyColors[need.urgency as keyof typeof urgencyColors] || urgencyColors.medium}`}>
                                            {need.urgency}
                                        </span>
                                    </div>
                                ...
                                <div className="p-5 flex-grow flex flex-col">
                                    ...
                                    <div className="mt-auto">
                                        ...
                                        <button onClick={() => setPledgingNeed(need)} disabled={isGoalMet}
                                            ...
                                            {isGoalMet ? 'Goal Met' : <>Pledge Donation <ArrowRight size={18} /></>}
                                        </button>
                                    </div>
                                </div>
                            </div>
            ...
            {pledgingNeed && (
                <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className="bg-slate-800 rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden border border-slate-700 transform transition-all animate-in fade-in zoom-in-95 duration-200">
                        <div className="relative h-32 bg-slate-900">
                            {pledgingNeed.imageBase64 ? (
                                <img src={pledgingNeed.imageBase64} className="w-full h-full object-cover opacity-40 mix-blend-overlay" />
                            ) : (
