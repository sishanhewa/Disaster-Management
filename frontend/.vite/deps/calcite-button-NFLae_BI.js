import { N as css, O as html, f as stringOrBoolean, j as nothing, l as safeClassMap, m as ref, p as createRef, r as customElement, s as LitElement } from "./runtime-C8rHe43j.js";
...
import { u as hasVisibleContent } from "./dom-DTFGtTyI.js";
...
import { i as findAssociatedForm, o as resetForm, s as submitForm } from "./form-Cp-QA3Rn.js";
import { t as useInteractive } from "./useInteractive-BqY0MsXy.js";
import { n as literal, t as html$1 } from "./static-html-C6s3b81L.js";
import { i as getLabelText, n as connectLabel, r as disconnectLabel } from "./label-BjTDb-rg.js";
//#region node_modules/@esri/calcite-components/dist/components/calcite-button/customElement.js
var CSS = {
...
var Button = class extends LitElement {
	...
	handleClick() {
		const { type } = this;
		if (this.href) return;
		if (type === "submit") submitForm(this);
		else if (type === "reset") resetForm(this);
	}
	setTooltipText() {
		const { contentRef: { value: contentEl } } = this;
		if (contentEl) this.tooltipText = contentEl.offsetWidth < contentEl.scrollWidth ? this.el.innerText || null : null;
	}
	setChildEl(el) {
	...
	render() {
		...
		const iconStartEl = html`<calcite-icon class=${safeClassMap({
			[CSS.icon]: true,
			[CSS.iconStart]: true
		})} .flipRtl=${this.iconFlipRtl === "start" || this.iconFlipRtl === "both"} .icon=${this.iconStart} .scale=${getIconScale(this.scale)}></calcite-icon>`;
		const iconEndEl = html`<calcite-icon class=${safeClassMap({
			[CSS.icon]: true,
			[CSS.iconEnd]: true
		})} .flipRtl=${this.iconFlipRtl === "end" || this.iconFlipRtl === "both"} .icon=${this.iconEnd} .scale=${getIconScale(this.scale)}></calcite-icon>`;
		...
		return this.interactiveContainer({
			...
			children: html$1`<${DynamicHtmlTag} .ariaBusy=${this.loading} .ariaExpanded=${this.el.ariaExpanded ? this.el.ariaExpanded : null} .ariaLabel=${!this.loading ? getLabelText(this) : this.messages.loading} aria-live=polite class=${safeClassMap({
...
//# sourceMappingURL=calcite-button-NFLae_BI.js.map