import { n as DEBOUNCE } from "./runtime-C8rHe43j.js";
...
//#endregion
//#region node_modules/composed-offset-position/dist/composed-offset-position.browser.mjs
...
function offsetParentPolyfill(element) {
	...
	for (let ancestor = flatTreeParent(element); ancestor; ancestor = flatTreeParent(ancestor)) {
		...
		if (style.position !== "static" || isContainingBlock(style)) return ancestor;
		if (ancestor.tagName === "BODY") return ancestor;
	}
	return null;
}
//#endregion
//#region node_modules/@esri/calcite-components/dist/chunks/floating-ui.js
...
var positionFloatingUI = (async (component, { referenceEl, floatingEl, overlayPositioning = "absolute", placement, flipDisabled, flipPlacements: flipPlacements2, offsetDistance, offsetSkidding, arrowEl, type }) => {
	...
	if (arrowEl && middlewareData.arrow) {
		...
		if ("floatingLayout" in component) component.floatingLayout = side === "left" || side === "right" ? "horizontal" : "vertical";
	...
	const visibility = middlewareData.hide?.referenceHidden ? "hidden" : null;
...
function getMiddleware({ placement, flipDisabled, flipPlacements: flipPlacements2, offsetDistance, offsetSkidding, arrowEl, type }) {
	const middleware = [shift(), hide()];
	if (type === "menu") middleware.push(flip({ fallbackPlacements: flipPlacements2 || [
		"top-start",
		"top",
		"top-end",
		"bottom-start",
	...
	if (placement === "auto" || placement === "auto-start" || placement === "auto-end") middleware.push(autoPlacement({ alignment: placement === "auto-start" ? "start" : placement === "auto-end" ? "end" : null }));
	else if (!flipDisabled) middleware.push(flip(flipPlacements2 ? { fallbackPlacements: flipPlacements2 } : {}));
...
function filterValidFlipPlacements(placements2, el) {
	...
	if (filteredPlacements.length !== placements2.length) console.warn(`${el.tagName}: Invalid value found in: flipPlacements. Try any of these: ${flipPlacements.map((placement) => `"${placement}"`).join(", ").trim()}`, { el });
	return filteredPlacements;
}
function getEffectivePlacement(placement, isRTL = false) {
	const placements2 = ["left", "right"];
	if (isRTL) placements2.reverse();
...
async function reposition(component, options, delayed = false) {
	if (!component.open || !options.floatingEl || !options.referenceEl) return;
	...
	if (!autoUpdatingComponentMap.get(component)) return runAutoUpdate(component);
	await (delayed ? getDebouncedReposition(component) : positionFloatingUI)(component, options);
}
function getDebouncedReposition(component) {
	...
	debounced = debounce(positionFloatingUI, DEBOUNCE.reposition, { edges: ["leading", "trailing"] });
...
async function runAutoUpdate(component) {
	...
	if (!floatingEl.isConnected) return;
	...
	const cleanUp = effectiveAutoUpdate(referenceEl, floatingEl, () => {
		...
		if (!repositionPromise) repositionPromise = promise;
	});
...
function disconnectFloatingUI(component) {
	...
	if (trackedState?.state === "active") trackedState.cleanUp();
...
//#endregion
export { defaultOffsetDistance as a, hideFloatingUI as c, defaultMenuPlacement as i, reposition as l, connectFloatingUI as n, disconnectFloatingUI as o, defaultEndMenuPlacement as r, filterValidFlipPlacements as s, FloatingCSS as t };

//# sourceMappingURL=floating-ui-DdeJyvwD.js.map