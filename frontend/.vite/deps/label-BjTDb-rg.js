import { d as isBefore, h as queryElementRoots, t as closestElementCrossShadowBoundary } from "./dom-DTFGtTyI.js";
//#region node_modules/@esri/calcite-components/dist/chunks/label.js
...
var findLabelForComponent = (componentEl) => {
	...
	if (!parentLabel || hasAncestorCustomElements(parentLabel, componentEl)) return null;
	return parentLabel;
};
function hasAncestorCustomElements(label, componentEl) {
	...
	return traversedElements.filter((el) => el !== componentEl && el !== label).filter((el) => el.tagName?.includes("-")).length > 0;
}
function connectLabel(component) {
	if (!component) return;
	const labelEl = findLabelForComponent(component.el);
	if (onLabelClickMap.has(labelEl) && labelEl === component.labelEl || !labelEl && unlabeledComponents.has(component)) return;
...
function onLabelClick(event) {
	...
	if (labelables.includes(clickedLabelable)) return;
	const firstLabelable = labelables[0];
	if (firstLabelable.disabled) return;
	firstLabelable.onLabelClick(event);
}
function onLabelConnected() {
	if (unlabeledComponents.has(this)) connectLabel(this);
}
...
async function associateExplicitLabelToUnlabeledComponent(label) {
	...
	if (labelToLabelables.has(label)) return;
	const forComponentEl = label.ownerDocument?.getElementById(label.for);
	if (!forComponentEl) return;
...
//#endregion
export { labelConnectedEvent as a, getLabelText as i, connectLabel as n, labelDisconnectedEvent as o, disconnectLabel as r, associateExplicitLabelToUnlabeledComponent as t };

//# sourceMappingURL=label-BjTDb-rg.js.map