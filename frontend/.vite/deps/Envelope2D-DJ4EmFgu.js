...
import { I as Ot, M as Ls, Y as Us, in as P, kt as mi, wn as z } from "./Point2D-ClM_Ex8K.js";
//#region node_modules/@arcgis/core/chunks/Envelope2D.js
var Envelope2D_exports = /* @__PURE__ */ __exportAll({ Envelope2D: () => n });
var n = class n {
	static construct(i, t, s, m) {
		return new n(i, t, s, m);
	}
	constructor(i, t, s, m) {
		this.m_EnvelopeType = 2, void 0 === i ? (this.xmin = NaN, this.ymin = NaN, this.xmax = NaN, this.ymax = NaN) : "number" == typeof i ? (this.xmin = i, this.ymin = t, this.xmax = s, this.ymax = m) : (this.xmin = i.xmin, this.ymin = i.ymin, this.xmax = i.xmax, this.ymax = i.ymax), this.normalize();
	}
	static constructEmpty() {
		return new n(NaN, NaN, NaN, NaN);
	}
	clone() {
		return new n(this);
	}
	...
	setEmpty() {
		this.xmin = NaN, this.ymin = NaN, this.xmax = NaN, this.ymax = NaN;
	}
	setInfinite() {
	...
	queryCorner(i) {
		switch (3 & i) {
			case 0: return new mi(this.xmin, this.ymin);
			case 1: return new mi(this.xmin, this.ymax);
			case 2: return new mi(this.xmax, this.ymax);
			default: return new mi(this.xmax, this.ymin);
		}
	}
	queryCorners(i) {
	...
	reaspect(i, t) {
		...
		const s = i / t, m = .5 * this.width(), h = .5 * this.height() * s;
		if (m <= h) {
			const i = this.getCenterX();
			this.xmin = i - h, this.xmax = i + h;
		} else {
			const i = m / s, t = this.getCenterY();
	...
	sqrDistanceEnvelope(i, t, s) {
		if (!t && !s) {
			if (this.isEmpty() || i.isEmpty()) return NaN;
			let t, s = 0, m = 0;
		...
		if (this.isEmpty() || i.isEmpty()) return t && t.setNAN(), s && s.setNAN(), NaN;
		let m, h = 0, x = 0;
		const n = Math.max(this.xmin, i.xmin), e = Math.max(this.ymin, i.ymin);
	...
	sqrMaxMinDistanceEnvelope(i) {
		...
		{
			const m = mi.sqrDistanceCoords(this.xmin, this.ymin, i.xmax, i.ymin), h = mi.sqrDistanceCoords(this.xmin, this.ymin, i.xmax, i.ymax), x = mi.sqrDistanceCoords(this.xmin, this.ymax, i.xmax, i.ymin), n = mi.sqrDistanceCoords(this.xmin, this.ymax, i.xmax, i.ymax);
			t = Math.min(Math.max(Math.max(m, h), Math.max(x, n)), t);
		}
		{
			const m = mi.sqrDistanceCoords(this.xmin, this.ymin, i.xmin, i.ymax), h = mi.sqrDistanceCoords(this.xmin, this.ymin, i.xmax, i.ymax), x = mi.sqrDistanceCoords(this.xmax, this.ymin, i.xmin, i.ymax), n = mi.sqrDistanceCoords(this.xmax, this.ymin, i.xmax, i.ymax);
			t = Math.min(Math.max(Math.max(m, h), Math.max(x, n)), t);
		}
		{
			const m = mi.sqrDistanceCoords(this.xmax, this.ymin, i.xmin, i.ymin), h = mi.sqrDistanceCoords(this.xmax, this.ymin, i.xmin, i.ymax), x = mi.sqrDistanceCoords(this.xmax, this.ymax, i.xmin, i.ymin), n = mi.sqrDistanceCoords(this.xmax, this.ymax, i.xmin, i.ymax);
			t = Math.min(Math.max(Math.max(m, h), Math.max(x, n)), t);
		}
		{
			const m = mi.sqrDistanceCoords(this.xmin, this.ymax, i.xmin, i.ymin), h = mi.sqrDistanceCoords(this.xmin, this.ymax, i.xmax, i.ymin), x = mi.sqrDistanceCoords(this.xmax, this.ymax, i.xmin, i.ymin), n = mi.sqrDistanceCoords(this.xmax, this.ymax, i.xmax, i.ymin);
			t = Math.min(Math.max(Math.max(m, h), Math.max(x, n)), t);
		}
		return t;
	}
	sqrMaxDistanceEnvelope(i) {
		...
		for (let m = 0; m < 4; m++) for (let i = 0; i < 4; i++) {
			const n = mi.sqrDistance(h[m], x[i]);
			n > t && (t = n);
		}
		return t;
	}
...
//#endregion
export { n, Envelope2D_exports as t };

//# sourceMappingURL=Envelope2D-DJ4EmFgu.js.map