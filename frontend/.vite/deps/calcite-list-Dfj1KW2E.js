import { N as css$1, O as html, T as createEvent, l as safeClassMap, m as ref, n as DEBOUNCE, p as createRef, r as customElement, s as LitElement } from "./runtime-C8rHe43j.js";
...
import { s as getRootNode, v as slotChangeHasAssignedElement } from "./dom-DTFGtTyI.js";
...
import "./label-BjTDb-rg.js";
import { i as numberStringFormatter } from "./locale-MFqIWoIv.js";
import "./calcite-scrim-B428rR7z.js";
import "./calcite-input-message-Cj7BiJaY.js";
import "./calcite-input-CpxXupa-.js";
import { a as listItemGroupSelector, c as updateListItemChildren, i as isListItem, o as listItemSelector, s as listSelector, t as expandedAncestors } from "./utils4-NppBZ6eF.js";
//#region node_modules/es-toolkit/dist/function/identity.mjs
function identity(x) {
	return x;
}
//#endregion
//#region node_modules/es-toolkit/dist/string/escapeRegExp.mjs
function escapeRegExp$1(str) {
	return str.replace(/[\\^$.*+?()[\]{}|]/g, "\\$&");
}
//#endregion
//#region node_modules/@esri/calcite-components/dist/chunks/resources8.js
var CSS$3 = {
	container: "container",
...
var Stack = class extends LitElement {
	...
	renderActionsStart() {
		...
		return keyed("actions-start-container", html`<div class=${safeClassMap(CSS$3.actionsStart)} .hidden=${!hasActionsStart}><slot name=${SLOTS$1.actionsStart} @slotchange=${this.handleActionsStartSlotChange}></slot></div>`);
	}
	renderActionsEnd() {
		const { hasActionsEnd } = this;
		return keyed("actions-end-container", html`<div class=${safeClassMap(CSS$3.actionsEnd)} .hidden=${!hasActionsEnd}><slot name=${SLOTS$1.actionsEnd} @slotchange=${this.handleActionsEndSlotChange}></slot></div>`);
	}
	renderContentStart() {
		const { hasContentStart } = this;
		return html`<div class=${safeClassMap(CSS$3.contentStart)} .hidden=${!hasContentStart}><slot name=${SLOTS$1.contentStart} @slotchange=${this.handleContentStartSlotChange}></slot></div>`;
	}
	renderDefaultContent() {
		return html`<div class=${safeClassMap(CSS$3.content)}><slot></slot></div>`;
	}
	renderContentEnd() {
		const { hasContentEnd } = this;
		return html`<div class=${safeClassMap(CSS$3.contentEnd)} .hidden=${!hasContentEnd}><slot name=${SLOTS$1.contentEnd} @slotchange=${this.handleContentEndSlotChange}></slot></div>`;
	}
	render() {
		return html`<div class=${safeClassMap(CSS$3.container)}>${this.renderActionsStart()}${this.renderContentStart()}${this.renderDefaultContent()}${this.renderContentEnd()}${this.renderActionsEnd()}</div>`;
	}
};
...
//#endregion
//#region node_modules/es-toolkit/dist/compat/object/forIn.mjs
...
//#endregion
//#region node_modules/es-toolkit/dist/compat/string/escapeRegExp.mjs
function escapeRegExp(str) {
	return escapeRegExp$1(toString(str));
}
//#endregion
//#region node_modules/@esri/calcite-components/dist/chunks/filter.js
var filter = (data, value, filterProps) => {
	const escapedValue = escapeRegExp(value);
	const regex = new RegExp(escapedValue, "i");
	if (value === "" || data.length === 0) return data;
	const find = (input, RE, fields) => {
		if (input?.filterDisabled) return true;
		let found = false;
		forIn(input, (val, key) => {
			if (typeof val === "function" || val == null) return;
			if (fields && !fields.includes(key)) return;
			if (Array.isArray(val) || typeof val === "object" && val !== null) {
				if (find(val, RE)) found = true;
...
var Filter = class extends LitElement {
	...
	willUpdate(changes) {
		if (changes.has("items") && (this.hasUpdated || this.items?.length > 0) || changes.has("filterProps")) this.filterDebounced(this.value);
	}
	valueHandler(value) {
	...
	updateFiltered(filtered, emit = false, callback) {
		...
		if (emit) this.calciteFilterChange.emit();
		callback?.();
	}
	render() {
...
//#endregion
//#region node_modules/@esri/calcite-components/dist/chunks/sortableComponent.js
...
function connectSortableComponent(component) {
	if (dragActive(component)) return;
	...
	component.sortable = Sortable.create(component.el, {
		...
		...!!group && {
			...
			group: {
				name: group,
				...!!component.canPull && { pull: (to, from, dragEl, { newDraggableIndex: newIndex, oldDraggableIndex: oldIndex }) => component.canPull({
					toEl: to.el,
					fromEl: from.el,
					dragEl,
					newIndex,
				...
				...!!component.canPut && { put: (to, from, dragEl, { newDraggableIndex: newIndex, oldDraggableIndex: oldIndex }) => component.canPut({
					toEl: to.el,
					fromEl: from.el,
					dragEl,
					newIndex,
		...
		onMove: ({ from: fromEl, dragged: dragEl, to: toEl, related: relatedEl }) => {
			if (!component.onDragMove) return;
...
//#endregion
//#region node_modules/@esri/calcite-components/dist/components/calcite-list/customElement.js
var CSS = {
	container: "container",
	table: "table",
	scrim: "scrim",
...
var List = class extends LitElement {
	...
	willUpdate(changes) {
		if (changes.has("filterText") || changes.has("filterProps") || changes.has("filterPredicate")) this.performFilter();
	...
	handleCalciteInternalFocusPreviousItem(event) {
		...
		const prevIndex = focusableItems.findIndex((listItem) => listItem.active) - 1;
		if (focusableItems[prevIndex]) this.focusRow(focusableItems[prevIndex]);
	}
	...
	handleSortReorder(event) {
		if (this.parentListEl || event.defaultPrevented) return;
		event.preventDefault();
	...
	handleCalciteInternalListItemSelect(event) {
		...
		if (target.selected && (selectionMode === "single" || selectionMode === "single-persist")) listItems.forEach((listItem) => listItem.selected = listItem === target);
		this.updateSelectedItems();
	}
	...
	setUpSorting() {
		...
		if (defaultSlotEl) updateListItemChildren(defaultSlotEl);
	...
	handleDefaultSlotChange() {
		if (this.parentListEl) this.calciteInternalListDefaultSlotChange.emit();
	}
	setListItemGroups() {
	...
	filterElements({ el, filteredItems, visibleParents }) {
		...
		if (!filterHidden) visibleParents.add(closestParent);
		this.filterElements({
			el: closestParent,
			filteredItems,
			visibleParents
	...
	allParentListItemsExpanded(item) {
		...
		else if (!parentItem.expanded) return false;
	...
	updateFilteredItems() {
		...
		lastDescendantItems.forEach((listItem) => this.filterElements({
			el: listItem,
			filteredItems,
			visibleParents
		}));
	...
	updateFilteredData() {
		const { filterEl } = this;
		if (!filterEl) return;
		if (filterEl.filteredItems) this.filteredData = filterEl.filteredItems;
		this.updateListItemsDebounced();
	}
	...
	getGroupHeading(item) {
		return this.listItemGroups.filter((group) => group.contains(item)).map((group) => group.heading);
	}
	updateGroupItems() {
		const { el, group, scale } = this;
		const rootNode = getRootNode(el);
		this.sortHandleMenuItems = (group ? Array.from(rootNode.querySelectorAll(`${listSelector}[group="${group}"]`)).filter((list) => !list.disabled && list.dragEnabled) : []).map((element) => ({
			element,
			label: element.label ?? element.id,
			id: guid()
		}));
		Array.from(this.el.querySelectorAll(listItemGroupSelector)).forEach((item) => {
			item.scale = scale;
		});
	}
	focusRow(focusEl) {
	...
	isNavigable(listItem) {
		...
		if (!parentListItemEl) return true;
		return parentListItemEl.expanded && this.isNavigable(parentListItemEl);
	}
	handleListKeydown(event) {
		if (event.defaultPrevented || !!this.parentListEl) return;
		const { key } = event;
		...
		if (key === "ArrowDown") {
			...
			if (navigableItems[nextIndex]) this.focusRow(navigableItems[nextIndex]);
		} else if (key === "ArrowUp") {
			event.preventDefault();
			...
			if (navigableItems[prevIndex]) this.focusRow(navigableItems[prevIndex]);
		} else if (key === "Home") {
			event.preventDefault();
			const homeItem = navigableItems[0];
			if (homeItem) this.focusRow(homeItem);
		} else if (key === "End") {
			event.preventDefault();
			const endItem = navigableItems[navigableItems.length - 1];
			if (endItem) this.focusRow(endItem);
		}
	}
	handleInteractionModeWarning() {
		if (this.interactionMode === "static" && this.selectionMode !== "none" && this.selectionAppearance === "border") console.warn(`selection-appearance="border" requires interaction-mode="interactive"`);
	}
	validateSortMenuItem({ fromEl, toEl, dragEl, newIndex, oldIndex, type }) {
		if (!fromEl || !toEl || toEl === fromEl || dragEl.contains(toEl)) return false;
	...
	handleAdd(event) {
		...
		const oldIndex = Array.from(fromEl.children).filter(isListItem).indexOf(dragEl);
		const newIndex = 0;
		if (!this.validateSortMenuItem({
			type: "add",
			fromEl,
			toEl,
			dragEl,
...
//# sourceMappingURL=calcite-list-Dfj1KW2E.js.map