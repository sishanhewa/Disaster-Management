import { A as Kt, cn as a, dn as f, et as W, in as P, k as K, kt as mi, mn as j$1, nt as Wt, wn as z } from "./Point2D-ClM_Ex8K.js";
import { R as mr, Ut as zt, b as Qs, i as Bh } from "./MultiPathImpl-Cj23glYA.js";
import { t as s } from "./GeometryCursor-4NZ0ZlkG.js";
import { t } from "./SimpleGeometryCursor-tjS0ieqv.js";
//#region node_modules/@arcgis/core/geometry/operators/gx/operatorOffset.js
function l(t, s, e, o, h, r) {
	if (null === t && P(""), t.getDimension() < 1 && P(""), h > 0 || P(""), j$1(t), 0 === s || t.isEmpty()) return t;
	const m = new A(r);
...
var y = 1.4142135623730951, x = .017453292519943295, p = 256, b = 512, M = 1024;
var g = class g {
...
var A = class A {
	...
	constructOffset() {
		...
		if (t === a.enumLine) return this.offsetLine();
		if (t === a.enumEnvelope) return this.offsetEnvelope();
		if (f(t)) {
			const t = new Qs();
	...
	offsetLine() {
		const t = this.m_inputGeometry, s = t.getStartXY(), e = t.getEndXY(), i = new mi();
	...
	addPointEx(t, s) {
		...
		const e = this.m_srcPtCount, i = this.m_srcPts[0 === s ? e - 1 : s - 1], n = this.m_srcPts[s], o = A.dotSign(i, n, this.m_offsetPts[this.m_offsetPtCount - 1], t);
		if (o > 0) this.addPoint(t);
		else if (o < 0) if (A.dotSign(i, n, n, this.m_offsetPts[this.m_offsetPtCount - 1]) > 0) {
			const n = new g();
			let o;
			o = 0 === s ? e - 2 : 1 === s ? e - 1 : s - 2;
			const h = this.m_srcPts[o], r = Math.atan2(i.y - h.y, i.x - h.x);
			if (A.buildPoint(i, this.m_distance, r - Wt, n), this.m_offsetPts[this.m_offsetPtCount - 1] = n.clone(), 1 === this.m_joins || 2 === this.m_joins) {
				n.x = .5 * (n.x + i.x), n.y = .5 * (n.y + i.y), this.addPoint(n), A.buildPoint(i, this.m_distance, this.m_a1 + Wt, n);
				const t = n.clone();
				t.x = .5 * (t.x + i.x), t.y = .5 * (t.y + i.y), t.type |= p, this.addPoint(t), this.addPoint(n);
			} else A.buildPoint(i, this.m_distance, this.m_a1 + Wt, n), n.type |= p, this.addPoint(n);
			this.addPointEx(t, s);
		} else {
			const t = new g();
			if (A.buildPoint(n, this.m_distance, this.m_a1 + Wt, t), this.addPoint(t), 1 === this.m_joins || 2 === this.m_joins) {
				t.x = .5 * (t.x + n.x), t.y = .5 * (t.y + n.y), this.addPoint(t), A.buildPoint(n, this.m_distance, this.m_a2 - Wt, t);
				const s = t.clone();
				s.x = .5 * (s.x + n.x), s.y = .5 * (s.y + n.y), s.type |= p, this.addPoint(s), this.addPoint(t);
			} else A.buildPoint(n, this.m_distance, this.m_a2 - Wt, t), t.type |= p, this.addPoint(t);
		}
	}
	buildOffset() {
		const t = new g(), s = this.m_srcPtCount;
		this.m_offsetPtCount = 0;
		const e = .5 * this.m_tolerance;
		let i = 0, n = 0;
		for (let o = 0; o < s; o++) {
			const h = this.m_srcPts[o], r = 0 === o ? this.m_srcPts[s - 1] : this.m_srcPts[o - 1], m = o === s - 1 ? this.m_srcPts[0] : this.m_srcPts[o + 1];
			let a, f, P = 0;
			{
				const t = r.x - h.x, s = r.y - h.y, e = m.x - h.x, _ = m.y - h.y;
				a = Math.atan2(s, t), f = Math.atan2(_, e), this.m_a1 = a, this.m_a2 = f, 0 === o && (i = a, n = f), P = mi.orientationRobust(h.asPoint2D(), r.asPoint2D(), m.asPoint2D());
			}
			const d = f;
			if (f < a && (f += Kt), !(Math.abs(Math.abs(a - f) - Math.PI) < 1e-8)) if (P * this.m_distance > 0) if (1 === this.m_joins || 2 === this.m_joins) {
				A.buildPoint(h, this.m_distance, a + Wt, t), this.addPoint(t);
				const s = .001;
				t.x = h.x + (t.x - h.x) * s, t.y = h.y + (t.y - h.y) * s, this.addPoint(t), A.buildPoint(h, this.m_distance, f - Wt, t);
				const e = t.clone();
				e.x = h.x + (e.x - h.x) * s, e.y = h.y + (e.y - h.y) * s, e.type |= p, this.addPoint(e), this.addPoint(t);
			} else {
				const s = .5 * (f - a), e = 0 === s ? this.m_distance : this.m_distance / Math.abs(Math.sin(s));
				A.buildPoint(h, e, .5 * (a + f), t), this.addPointEx(t, o);
			}
			else {
				if (h.type & b) {
					const s = 1 - e / Math.abs(this.m_distance);
					let i = 1, n = this.m_distance < 0 ? -Math.PI : Math.PI;
					if (s > -1 && s < 1) {
						let t = 2 * Math.acos(s);
					...
					A.buildPoint(h, this.m_distance, f - Wt, t), t.type |= M, this.addPoint(t);
					continue;
				}
				if (1 !== this.m_joins) {
					if (0 === this.m_joins) {
					...
					if (2 === this.m_joins) {
						const s = r.x - h.x, e = r.y - h.y, i = m.x - h.x, n = m.y - h.y;
						let _ = (s * i + e * n) / Math.sqrt(s * s + e * e) / Math.sqrt(i * i + n * n);
						if (_ = Math.max(_, -1), _ > .99999999) {
						...
						const u = Math.abs(this.m_distance / Math.sin(.5 * Math.acos(_))), P = Math.abs(this.m_miterLimit * this.m_distance);
						if (u > P) {
							const s = .5 * (f - a), e = this.m_distance / Math.abs(Math.sin(s));
							A.buildPoint(h, e, .5 * (a + f), t);
							const i = mi.construct(t.x, t.y), n = mi.construct(h.x, h.y), r = new mi();
							r.setSub(i, n);
							const m = new mi();
							m.setScaleAdd(P / r.length(), r, n);
							const _ = Math.sqrt(u * u - this.m_distance * this.m_distance), d = (u - P) * Math.abs(this.m_distance) / _;
							this.m_distance > 0 ? r.rightPerpendicularThis() : r.leftPerpendicularThis(), r.scale(d / r.length());
							const l = new mi();
							l.setAdd(m, r);
							const y = new mi();
						...
						const d = .5 * (f - a), l = this.m_distance / Math.abs(Math.sin(d));
						A.buildPoint(h, l, .5 * (a + f), t), this.addPointEx(t, o);
						continue;
					}
					{
						let s;
						if (f = d, this.m_distance > 0 ? (f > a && (f -= Kt), s = a - f < Wt) : (f < a && (f += Kt), s = f - a < Wt), s) {
							...
							const s = .5 * (f - a), e = this.m_distance / Math.abs(Math.sin(s));
							f < a && (f += Kt), A.buildPoint(h, e, (a + f) / 2, t), this.addPointEx(t, o);
						}
					}
				} else A.buildPoint(h, this.m_distance, a + Wt, t), this.addPointEx(t, o), A.buildPoint(h, this.m_distance, f - Wt, t), this.addPoint(t);
			}
		}
	...
	handleClosedIntersection(t, s) {
		const e = this.m_offsetPts[this.m_offsetPts[t].m_prev], i = this.m_offsetPts[t], n = this.m_offsetPts[this.m_offsetPts[s].m_prev], o = this.m_offsetPts[s];
		...
		if (((i.x - e.x) * (o.y - n.y) - (i.y - e.y) * (o.x - n.x)) * this.m_distance < 0) {
			if (this.findIntersection(e, i, n, o, h) && !h.bAtExistingPt) {
				const r = 1e-8, m = Math.sqrt((i.x - e.x) * (i.x - e.x) + (i.y - e.y) * (i.y - e.y)), a = (i.x - e.x) / m, f = (i.y - e.y) / m, c = Math.sqrt((o.x - n.x) * (o.x - n.x) + (o.y - n.y) * (o.y - n.y)), _ = (o.x - n.x) / c, u = (o.y - n.y) / c;
				let P = !1;
				const d = h.pt.clone();
				d.x += (a + _) * r, d.y += (f + u) * r;
				let l = n, y = h.pt, x = this.m_offsetPts[t].m_prev;
				for (; y.y > d.y != l.y > d.y && d.x < (l.x - y.x) * (d.y - y.y) / (l.y - y.y) + y.x && (P = !P), l = y, x = this.m_offsetPts[x].m_next, x !== s;) y = this.m_offsetPts[x];
				if (P) return -1;
	...
	offsetMultiPath(t) {
		const s = Bh(this.m_inputGeometry, 0, this.m_tolerance, 0, this.m_progressTracker, 12e3), e = s.querySegmentIterator();
		e.resetToFirstPath();
		let i = -1;
		for (; e.nextPath();) i++, this.offsetPath(s, i, t);
	}
	offsetPath(t, s, e) {
		let i = t.getPathStart(s), n = t.getPathEnd(s);
	...
	static dotSign(t, s, e, i) {
		return K(W(s.x - t.x, s.y - t.y, i.x - e.x, i.y - e.y));
	}
};
var E = class {
	getOperatorType() {
...
var v = class extends s {
	...
	offset(t) {
		let s;
		return j$1(t), s = this.m_flattenError <= 0 ? zt(this.m_spatialReference, t, !0).total() : this.m_flattenError, l(t, this.m_distance, this.m_joins, this.m_miterLimit, s, this.m_progressTracker);
	}
};
var G = new E();
...
function j(s, e, i, n, o, h) {
	const r = G.executeMany(new t(s), e, i, n, o, h, null);
	return Array.from(r);
}
//#endregion
export { w as n, j as t };

//# sourceMappingURL=operatorOffset-BWN-mpyq.js.map