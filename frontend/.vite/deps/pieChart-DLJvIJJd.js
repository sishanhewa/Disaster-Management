...
import { a as cos, c as max, d as sin, f as sqrt, i as atan2, l as min, n as acos, p as tau, r as asin, s as halfPi, t as abs, u as pi } from "./math-ibC1KsGN.js";
import { $t as p50, A as normalizeAngle, E as cos$1, F as ListTemplate, Gt as isNumber, M as sin$1, Mt as each, O as getArcBounds, Qt as p100, Xt as Percent, d as Graphics, en as percent, f as visualSettings, ft as relativeToValue, i as Container, k as mergeBounds, lt as mergeTags, m as Template, n as Label, r as Text, t as Theme, w as RADIANS } from "./Theme-kaw1IGF4.js";
import { n as setColor } from "./DefaultTheme-D93gZRcP.js";
import { n as SerialChart, r as Series, t as Tick } from "./Tick-CcwcFRd0.js";
...
//#endregion
//#region node_modules/@amcharts/amcharts5/.internal/core/render/RadialText.js
/**
* @ignore
*/
...
//#endregion
//#region node_modules/@amcharts/amcharts5/.internal/core/render/RadialLabel.js
var RadialLabel = class extends Label {
	...
	*/
	baseRadius() {
		...
		return innerRadiusPrivate + relativeToValue(this.get("baseRadius", 0), radiusPrivate - innerRadiusPrivate);
	}
	/**
	...
	_updatePosition() {
		...
		if (textType == "radial") {
			if (this._flipped) {
				...
				if (centerX instanceof Percent) w = w * (1 - centerX.value * 2);
...
//#endregion
//#region node_modules/@amcharts/amcharts5/.internal/core/render/Slice.js
/**
* Draws a slice shape.
*
...
var Slice = class extends Graphics {
	...
	_getTooltipPoint() {
		...
		let x = 0;
		let y = 0;
		if (isNumber(tooltipX)) x = tooltipX;
		if (isNumber(tooltipY)) y = tooltipY;
		...
		if (innerRadius < 0) innerRadius = radius + innerRadius;
		if (tooltipX instanceof Percent) x = this.ix * (innerRadius + (radius - innerRadius) * tooltipX.value);
		if (tooltipY instanceof Percent) y = this.iy * (innerRadius + (radius - innerRadius) * tooltipY.value);
	...
	_beforeChanged() {
		super._beforeChanged();
		if (this.isDirty("radius") || this.isDirty("arc") || this.isDirty("innerRadius") || this.isDirty("startAngle") || this.isDirty("dRadius") || this.isDirty("dInnerRadius") || this.isDirty("cornerRadius") || this.isDirty("shiftRadius")) this._clear = true;
	}
	_changed() {
		super._changed();
		if (this._clear) {
			...
			if (arc > .1) generator.cornerRadius(this.get("cornerRadius", 0));
...
//#endregion
//#region node_modules/@amcharts/amcharts5/.internal/charts/percent/PercentDefaultTheme.js
/**
* @ignore
*/
var PercentDefaultTheme = class extends Theme {
	setupDefaultRules() {
		...
		*/
		...
		r("FunnelSlice").states.create("hover", { expandDistance: .15 });
		...
		*/
		...
		r("FunnelSlice", ["pyramid", "link"]).setAll({ fillOpacity: .5 });
		r("FunnelSlice", [
			"pyramid",
			"link",
			"vertical"
		...
		r("FunnelSlice", ["pyramid"]).states.create("hover", { expandDistance: .15 });
		...
		*/
		...
		r("Tick", ["pictorial"]).setAll({ location: .5 });
		{
			const rule = r("Graphics", ["pictorial", "background"]);
			rule.setAll({ fillOpacity: .2 });
			setColor(rule, "fill", ic, "alternativeBackground");
		}
	}
};
//#endregion
//#region node_modules/@amcharts/amcharts5/.internal/charts/percent/PercentChart.js
/**
...
//#endregion
//#region node_modules/@amcharts/amcharts5/.internal/charts/percent/PercentSeries.js
/**
...
var PercentSeries = class extends Series {
	...
	*/
	...
	_shouldMakeBullet(dataItem) {
		if (dataItem.get("value") != null) return true;
		return false;
	}
	/**
	* @ignore
	...
	*/
	_updateChildren() {
		super._updateChildren();
		if (this._valuesDirty) each(this._dataItems, (dataItem) => {
			dataItem.get("label").text.markDirtyText();
		});
		if (this.isDirty("legendLabelText") || this.isDirty("legendValueText")) each(this._dataItems, (dataItem) => {
	...
	processDataItem(dataItem) {
		...
		if (dataItem.get("fill") == null) {
			...
			if (colors) dataItem.setRaw("fill", colors.next());
		}
	...
	*/
	showDataItem(dataItem, duration) {
		...
		return __awaiter(this, void 0, void 0, function* () {
			...
			if (animation) promises.push(animation.waitForStop());
			const tick = dataItem.get("tick");
			if (tick) promises.push(tick.show(duration));
			const label = dataItem.get("label");
			if (label) promises.push(label.show(duration));
			const slice = dataItem.get("slice");
			if (slice) promises.push(slice.show(duration));
			if (slice.get("active")) slice.states.applyAnimate("active");
			yield Promise.all(promises);
		});
	}
	/**
	...
	*/
	hideDataItem(dataItem, duration) {
		...
		return __awaiter(this, void 0, void 0, function* () {
			...
			if (animation) promises.push(animation.waitForStop());
			const tick = dataItem.get("tick");
			if (tick) promises.push(tick.hide(duration));
			const label = dataItem.get("label");
			if (label) promises.push(label.hide(duration));
			const slice = dataItem.get("slice");
			slice.hideTooltip();
			if (slice) promises.push(slice.hide(duration));
			yield Promise.all(promises);
		});
	}
	/**
	...
	*/
	hoverDataItem(dataItem) {
		const slice = dataItem.get("slice");
		if (slice && !slice.isHidden()) slice.hover();
	}
	/**
			...
			if (slice) {
				...
				if (legendDataItem) {
					...
					each(visualSettings, (setting) => {
						if (slice.get(setting) != null) markerRectangle.set(setting, slice.get(setting));
					});
				}
			}
		}
	...
	_arrangeDown(labels) {
		if (labels) {
			...
			each(labels, (l) => {
				...
				if (l.y + labelTop < next) l.y = next - labelTop;
				l.label.set("y", l.y);
	...
	_arrangeUp(labels) {
		if (labels) {
			...
			each(labels, (l) => {
				...
				if (l.y + labelBottom > next) l.y = next - labelBottom;
				l.label.set("y", l.y);
				next = l.y + bounds.top;
			});
		}
	...
	_arrangeRight(labels) {
		if (labels) {
			...
			each(labels, (l) => {
				...
				if (l.y + labelLeft < next) l.y = next - labelLeft;
				l.label.set("x", l.y);
	...
	_arrangeLeft(labels) {
		if (labels) {
			...
			each(labels, (l) => {
				...
				if (l.y + labelRight > next) l.y = next - labelRight;
				l.label.set("x", l.y);
...
//#endregion
//#region node_modules/@amcharts/amcharts5/.internal/charts/pie/PieChart.js
/**
* Creates a pie chart.
*
...
var PieChart = class extends PercentChart {
	...
	_prepareChildren() {
		...
		if (this.isDirty("startAngle") || this.isDirty("endAngle") || prevRadius != this._maxRadius) this.series.each((series) => {
			series._markDirtyKey("startAngle");
		});
		if (this.isDirty("innerRadius") || this.isDirty("radius")) this.series.each((series) => {
	...
	*/
	radius(series) {
		...
		if (series) {
			...
			if (seriesRadius != null) return innerRadius + relativeToValue(seriesRadius, radius - innerRadius);
			else return innerRadius + (radius - innerRadius) / length * (index + 1);
		}
		return radius;
	}
	/**
	...
	*/
	innerRadius(series) {
		...
		if (series) {
			...
			if (seriesInnerRadius != null) return innerRadius + relativeToValue(seriesInnerRadius, radius - innerRadius);
			else return innerRadius + (radius - innerRadius) / length * index;
		}
		return innerRadius;
	}
	_updateSize() {
...
//#endregion
//#region node_modules/@amcharts/amcharts5/.internal/charts/pie/PieSeries.js
/**
...
var PieSeries = class extends PercentSeries {
	_makeSlices() {
		return new ListTemplate(Template.new({}), () => Slice._new(this._root, { themeTags: mergeTags(this.slices.template.get("themeTags", []), ["pie", "series"]) }, [this.slices.template]));
	}
	_makeLabels() {
		return new ListTemplate(Template.new({}), () => RadialLabel._new(this._root, { themeTags: mergeTags(this.labels.template.get("themeTags", []), ["pie", "series"]) }, [this.labels.template]));
	}
	_makeTicks() {
		return new ListTemplate(Template.new({}), () => Tick._new(this._root, { themeTags: mergeTags(this.ticks.template.get("themeTags", []), ["pie", "series"]) }, [this.ticks.template]));
	}
	processDataItem(dataItem) {
		super.processDataItem(dataItem);
	...
	_getNextUp() {
		const chart = this.chart;
		if (chart) return chart._maxRadius;
	...
	_prepareChildren() {
		...
		if (chart) {
			if (this.isDirty("alignLabels")) {
				...
				if (this.get("alignLabels")) labelsTemplate.set("textType", "aligned");
				else {
					let textType = labelsTemplate.get("textType");
					if (textType == null || textType == "aligned") labelsTemplate.set("textType", "adjusted");
				}
			}
			if (this._valuesDirty || this.isDirty("radius") || this.isDirty("innerRadius") || this.isDirty("startAngle") || this.isDirty("endAngle") || this.isDirty("alignLabels")) {
				...
				const arc = this.get("endAngle", chart.get("endAngle", 270)) - startAngle;
					...
					if (label) {
						...
						if (label.get("textType") == "aligned") {
							...
							if (middleAngle > 90 && middleAngle <= 270) {
								if (!label.isHidden() && !label.isHiding()) this._lLabels.push({
									label,
									y
								});
								labelRadius *= -1;
								...
								if (!label.isHidden() && !label.isHiding()) this._rLabels.push({
									label,
									y
								});
	...
	_updateTick(dataItem) {
		...
		if (tick && label && slice) {
			...
			if (x != 0 || y != 0) {
				...
				let dx = -pr;
				if (label.getPrivate("left")) dx = pl;
				points = [
					{
	...
	_positionBullet(bullet) {
		...
		if (sprite) {
			...
			if (slice) {
				...
				const locationX = bullet.get("locationX", .5);
				const locationY = bullet.get("locationY", .5);
...
//#endregion
export { PieChart as PieChartAm5, PieSeries as PieSeriesAm5 };

//# sourceMappingURL=pieChart-DLJvIJJd.js.map