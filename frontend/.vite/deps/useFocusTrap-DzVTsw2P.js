import { a as getConfig } from "./runtime-C8rHe43j.js";
import { n as makeGenericController } from "./controllers-2rrOeKHA.js";
import { b as tabbableOptions, r as focusElement } from "./dom-DTFGtTyI.js";
...
//#endregion
//#region node_modules/@esri/calcite-components/dist/chunks/useFocusTrap.js
function getEffectiveContainerElements(targetEl, { focusTrapOptions }, extraContainers) {
	if (!focusTrapOptions?.extraContainers && !extraContainers) return targetEl;
	return [
		targetEl,
		...toContainerArray(focusTrapOptions?.extraContainers),
		...toContainerArray(extraContainers)
	];
}
...
function defaultSetReturnFocus(hostEl, el) {
	...
	if (!outsideClickDeactivated.has(hostEl) && hasPreviousRelatedFocusedEl) focusElement(el);
	return false;
}
function createFocusTrapOptions(hostEl, options) {
	...
	return {
		...
		clickOutsideDeactivates: (event) => {
			if (!outsideClickDeactivated.has(hostEl)) outsideClickDeactivated.add(hostEl);
...
var useFocusTrap = (options) => {
	return makeGenericController((component, controller) => {
		...
		controller.onConnected(() => {
			if (component[options.triggerProp] && focusTrap) utils.activate();
		});
		controller.onUpdate((changes) => {
			if (component.hasUpdated && changes.has("focusTrapDisabled")) if (component.focusTrapDisabled) utils.deactivate();
			else utils.activate();
		});
		...
		const utils = {
			...
			activate: () => {
				...
				if (!focusTrap) {
					...
					focusTrap = createFocusTrap(effectiveContainers, createFocusTrapOptions(targetEl, {
						...internalFocusTrapOptions,
						...component.focusTrapOptions
					}));
				}
				if (typeof component.focusTrapDisabledOverride === "function" ? !component.focusTrapDisabledOverride() : !component.focusTrapDisabled) focusTrap.activate();
			},
			deactivate: () => focusTrap?.deactivate(),
			overrideFocusTrapEl: (el) => {
				if (focusTrap) throw new Error("Focus trap already created");
				focusTrapEl = el;
			},
			setExtraContainers: (extraContainers) => {
				effectiveContainers = getEffectiveContainerElements(focusTrapEl || component.el, component, extraContainers);
			},
...
//# sourceMappingURL=useFocusTrap-DzVTsw2P.js.map